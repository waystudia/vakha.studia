...
export function PublicOrderPage() {
  ...
  return <main className="public-page"><div className="public-shell"><header className="public-header"><div className="public-logo"><span className="brand-mark">VS</span><div><strong>Vakha Studio</strong><small>Школьная фотография</small></div></div><span className="step-indicator">{step === "child" ? "Шаг 1 из 2" : step === "services" ? "Шаг 2 из 2" : "Готово"}</span></header>
    ...
    {previewItem && <Modal title={previewItem.name} onClose={() => setPreviewItem(null)}><div className="public-media-viewer">{previewItem.preview_video_url ? <video controls playsInline poster={previewItem.preview_url || undefined} src={previewItem.preview_video_url} /> : previewItem.preview_url ? <img src={previewItem.preview_url} alt={previewItem.name} /> : <ServiceIcon type={previewItem.type} />}</div><div className="viewer-meta"><strong>{money(previewItem.price)}</strong></div></Modal>}
