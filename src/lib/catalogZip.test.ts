...
describe("CRM Photo catalog ZIP", () => {
  it("читает услуги и превью из реального формата catalog_export.zip", async () => {
    const file = await catalogFile({ services: [{ id: "photo", name: "Обычные фото", price: 400, gender: "girls", shortDescription: "Школьные снимки", previewImage: "assets/images/photo.png", presentation: { parentPreviewMode: "print" } }] });
    ...
    expect(items[0].parentPreviewMode).toBe("print");
  });
